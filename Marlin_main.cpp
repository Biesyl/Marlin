...
// G30 - Experimantal Calibrate print surface with automatic Z probe by Drukarze3D - Biesyl
...
// M666 - Set the distance from endstops by Drukarze3D
...
// Drukarze3D - Biesyl
#ifdef DELTA
  float endstop_adj[3]={X_END_ADJ,Y_END_ADJ,Z_END_ADJ};
#endif
...
static float bed_level2 [BED_LVL_SIZE] [BED_LVL_SIZE]; 
...
void setup()
{
  ...
  //zero the bed_level table
  for (int i =0; i<BED_LVL_SIZE; i++)
    for (int j = 0; j<BED_LVL_SIZE; j++)
      bed_level2[i][j] =0;
}
...
void deploy_z_probe() {
  ...
  destination[Y_AXIS] = 162;
  destination[Z_AXIS] = 50;
  prepare_move_raw();

  feedrate = homing_feedrate[X_AXIS]/10;
  destination[X_AXIS] = -15; //was -10
  prepare_move_raw();
  st_synchronize();
}
...
float z_probe() {
  ...
  feedrate = homing_feedrate[Z_AXIS];
  destination[Z_AXIS] = mm+3; //// Biesyl back to 3 ; Smoluch changed from 2 to 3 for extra glass sheet not full size
  prepare_move_raw();
  return mm;
}

void calibrate_print_surface2(float z_offset) {
  
  //Drukarze3d - Biesyl
  int center_point =  floor(BED_LVL_SIZE / 2 ); // Middle of the hexagon 
  ...
  for (int y = center_point; y >= -center_point; y--) {
    ...
    for (int x = -center_point*dir; x != (center_point +1 ) * dir; x += dir) {
      ...
      else { //outside of the hexagon (remaining points)
        bed_level2[x+center_point][y+center_point] = 0.0;
      }
    }
    ...
    // Print calibration results for manual frame adjustment.
      for (int x = -center_point; x <= center_point; x++) {
        SERIAL_PROTOCOL_F(bed_level2[x+center_point][y+center_point], 3);
        SERIAL_PROTOCOLPGM(" ");
      }
    }
    SERIAL_ECHOLN("");
...
void process_commands()
{
  ...
  if(code_seen('G'))
  {
    ...
    case 30: // G30 Eperimantal Calibrate print surface with automatic Z probe.
      saved_feedrate = feedrate;
      saved_feedmultiply = feedmultiply;
      feedmultiply = 100;

      deploy_z_probe();
      calibrate_print_surface2(z_probe_offset[Z_AXIS] +
  (code_seen(axis_codes[Z_AXIS]) ? code_value() : 0.0));
      retract_z_probe();
...
// Adjust print surface height by linear interpolation over the bed_level array. Drukarze3d - Biesyl
void adjust_delta2(float cartesian[3])
{
  int center_point =  floor(BED_LVL_SIZE / 2 ); // center point of the hexagon  
  int center_point_plus_1 = center_point +1;
  float grid_x = max(-center_point, min(center_point, cartesian[X_AXIS] / AUTOLEVEL_SIZE_GRID));
  float grid_y = max(-center_point, min(center_point, cartesian[Y_AXIS] / AUTOLEVEL_SIZE_GRID));
  int floor_x = floor(grid_x);
  int floor_y = floor(grid_y);
  float ratio_x = grid_x - floor_x;
  float ratio_y = grid_y - floor_y;
  float z1 = bed_level2[floor_x+center_point][floor_y+center_point];
  float z2 = bed_level2[floor_x+center_point][floor_y+center_point_plus_1];
  float z3 = bed_level2[floor_x+center_point_plus_1][floor_y+center_point];
  float z4 = bed_level2[floor_x+center_point_plus_1][floor_y+center_point_plus_1];
...
void prepare_move_with_mode(int mode )
{
